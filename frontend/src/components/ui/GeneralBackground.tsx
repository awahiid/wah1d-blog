...
export function GeneralBackground({children, className}: { children: React.ReactNode, className?: string }) {
    return (
        <div className={'w-full bg-neutral relative'}>
            ...
            <div className={'max-w-7xl relative mx-auto border border-transparent h-full'}>
                <div className={cn(className, 'max-w-7xl relative mx-auto border border-transparent h-full')}>
                    {children}
                </div>
            </div>
        </div>
